...
def start_pygame_window(text):
    pygame.init()
    WIDTH, HEIGHT = 800, 600
    pygame.mixer.init()
    pygame.mixer.music.load('firework_sound.mp3')
    ...
    def main():
        ...
        while run:
            draw_text(text, text_font, (255, 255, 255), 180, 120)
            pygame.display.flip()
            clock.tick(FPS)
            ...
            draw(launchers)
            if pygame.mixer.music.get_busy() == 0 and time.time() - start_time > 2:
...
def open_pygame_window():
    user_text = entry.get()
    root.destroy()
    start_pygame_window(user_text)

# Create the main tkinter window
root = tk.Tk()
root.title("Input Celebration Text")
root.geometry("500x250")
root.resizable(False, False)
root.config(bg='light blue')

# Create a label and entry for input
label = tk.Label(root, text="Enter your celebration text:",
...
button = tk.Button(root, text="Celebrate!", command=open_pygame_window,
                   font='arial 12 italic bold', bg='blue',fg='white')
button.place(x=195,y=135)

# instagram page
insta_page=tk.Label(root,text="@pythonagham",bg='light blue',
              fg='blue',font='arial 11 bold ')
insta_page.place(x=185,y=210)
